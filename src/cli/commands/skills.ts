...
configuration directory; workspace Skills live in .agents/skills and must be trusted
...
`);
  ...
  command.command("trust").description("Trust the current workspace's .agents/skills content until it changes").action(async () => {
  ...
  command.command("untrust").description("Remove trust for the current workspace's .agents/skills directory").action(async () => {
    const directory = await workspaceSkillsDirectory();
    const config = await store.loadConfig();
